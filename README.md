...
├─ private/                # OpenStack, Private K8s, GPU Worker, Storage 구현
│  ├─ openstack/
...
| 구분 | 경로 |
| --- | --- |
| GitHub Actions 실행 파일 | `.github/workflows/private-cloud-{plan,controller,remote}.yml` |
| OpenStack Terraform | `private/openstack` |
| OpenStack Kubernetes bootstrap | `private/kubernetes-bootstrap` |
| Kubernetes 기본 리소스 | `private/kubernetes` |
| Storage 기본 리소스 | `private/storage` |
| GPU Worker 검증 리소스 | `private/gpu-worker` |
| GitLab GPU runner 토대 | `private/openstack`, `private/handoff` |
| Caddy reverse proxy / DNS | `private/reverse-proxy` |
| 역할 간 인계 문서 | `private/handoff` |

## Project CLI

프로젝트 전용 CLI는 repository root의 `ha` 스크립트로 관리합니다.
...
- `ha proxy validate`: Caddy 기반 관리자 진입점 설정을 확인합니다.
...
물리 서버는 Tailscale VPN으로 접근하고, Caddy가 관리자 HTTP/HTTPS 진입점을 분기합니다.
