...
- `Private Cloud Plan` workflow를 실행합니다.
- network, subnet, security group, key pair, VM resource 계획을 확인합니다.
- 문제가 없으면 `Private Cloud Apply` workflow를 실행합니다.
...
- `destroy` 전 Kubernetes resource cleanup이 필요하면 `PRIVATE_CLOUD_KUBECONFIG_B64`를 GitHub Secrets에 등록합니다.

## Cluster 기본 리소스

- `apply_kubernetes=true`로 namespace, quota, RBAC, network policy를 적용합니다.
- `setup_storage=true`로 NFS export, MinIO, NFS provisioner를 적용합니다.
- `validate_gpu=true`로 RuntimeClass, device plugin, GPU validation job을 실행합니다.
