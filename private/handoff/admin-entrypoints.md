...
현재 Kubernetes Dashboard는 설치하지 않았습니다. Kubernetes 관리는 SSH와 `kubectl`로 수행합니다.
...
sudo kubectl --kubeconfig=/etc/kubernetes/admin.conf get ns
```
...
| kubeadm | OpenStack VM 위에 Kubernetes cluster를 bootstrap하는 도구 |
