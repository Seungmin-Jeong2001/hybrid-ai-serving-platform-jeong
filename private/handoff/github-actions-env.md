...
| `CONTROL_PLANE_FLAVOR_NAME` | `TF_VAR_control_plane_flavor_name` | control-plane VM flavor, 기본 `m1.large` |
...
| `PRIVATE_CLOUD_DNS_SERVICES` | `HA_DNS_SERVICES` | CNAME 대상 서비스 목록, 기본 `openstack,k8s,grafana,argocd,gitlab`. DNS 스크립트는 `gitlab`을 항상 보장하고, legacy `git` 값은 `gitlab`으로 정규화 |
